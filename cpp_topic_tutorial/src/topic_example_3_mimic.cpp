...
private:
  ...
  void sub_callback(const Twist::SharedPtr msg) {
    twist_pub_msg.linear.x = msg->linear.x;
    twist_pub_msg.angular.z = msg->angular.z;

    twist_publisher->publish(twist_pub_msg);
  }

public:
  MimicNode() : Node("mimic_node") {
    twist_subscriber = create_subscription<Twist>(
      "turtle1/cmd_vel", 10,
      std::bind(&MimicNode::sub_callback, this, std::placeholders::_1)
    );
    twist_publisher = create_publisher<Twist>("turtle2/cmd_vel", 10);
  }
};
